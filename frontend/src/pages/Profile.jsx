...
export default function Profile() {
  ...
  // EFFECT: Fetch LISTENING TIME stats (re-fetch when timeframe changes)
  useEffect(() => {
    if (!user) return;
    ...
    const fetchListeningStats = async () => {
      try {
        ...
        const listenRes = await fetch(`${API}/stats/listening?period=${period}`, {
          headers,
          signal: abortController.signal,
        });
        const listenData = await listenRes.json();

        if (!abortController.signal.aborted && listenData.success && listenData.data) {
  ...
  // EFFECT: Fetch TOP SONGS and TOP ARTISTS (mount-only — these are lifetime stats)
  useEffect(() => {
    if (!user) return;
    let isMounted = true;
    ...
    fetchTopStats();
    return () => { isMounted = false; };
  }, [user]); // Only re-run when user changes, NOT on timeframe change
